...
export default function AdminDashboard() {
  ...
  return <AppLayout mainClass="sa-main app-main-scroll"><div className="sa-page">
    <header className="sa-heading"><div><h1>Administración del sistema</h1><p>Supervisión de accesos, seguridad y disponibilidad.</p><small className="sa-freshness"><span/>Actualización automática cada 30 s · {metricas.generadoEn ? new Date(metricas.generadoEn).toLocaleTimeString('es-MX') : 'sin sincronizar'}</small></div><div className="sa-heading-actions"><div className="sa-window">{[7,30,90].map(value => <button key={value} className={dias===value?'is-active':''} onClick={()=>setDias(value)}>{value} días</button>)}</div><button onClick={actualizarTodo} disabled={actualizando} aria-busy={actualizando}>{actualizando?'Actualizando…':'Actualizar'}</button></div></header>
    <nav className="sa-dashboard-tabs" aria-label="Secciones del dashboard">{[['resumen','Resumen'],['red','Red y seguridad'],['actividad','Actividad']].map(([key,label])=><button key={key} className={vista===key?'is-active':''} onClick={()=>setVista(key)}>{label}</button>)}</nav>
