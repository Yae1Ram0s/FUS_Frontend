...
export default function AdminOTP() {
  ...
  return <AppLayout mainClass="sa-main app-main-scroll"><div className="sa-page">
    <header className="sa-heading"><div><h1>Supervisión de códigos OTP</h1><p>Confirma entregas para primer acceso y recuperación sin mostrar códigos sensibles.</p></div><button onClick={reload} disabled={loading} data-analytics-event="INTERACTION" data-analytics-component="ADMIN_OTP_ACTUALIZAR">Actualizar</button></header>
    ...
    <section className="sa-filters sa-filters--otp"><input aria-label="Buscar destinatario" placeholder="Buscar correo institucional…" value={filtros.search} onChange={e => campo('search', e.target.value)} data-analytics-event="INTERACTION" data-analytics-component="ADMIN_OTP_FILTROS" data-analytics-action="SEARCH" data-analytics-trigger="change" /><select aria-label="Estado del envío" value={filtros.estado} onChange={e => campo('estado', e.target.value)} data-analytics-event="INTERACTION" data-analytics-component="ADMIN_OTP_FILTROS" data-analytics-action="FILTER" data-analytics-trigger="change" data-analytics-metadata="estado"><option value="">Todos los estados</option><option value="ENVIADO">Enviados</option><option value="ERROR">Con error</option><option value="SIN_CONFIRMACION">Sin confirmación</option></select></section>
    {error && <div className="sa-error">{mensajeErrorAdmin(error)} <button onClick={reload} data-analytics-event="INTERACTION" data-analytics-component="ADMIN_OTP_REINTENTAR">Reintentar</button></div>}
    ...
    <nav className="sa-pagination" aria-label="Paginación"><button disabled={filtros.page <= 1} onClick={() => setFiltros({ ...filtros, page: filtros.page - 1 })} data-analytics-event="INTERACTION" data-analytics-component="ADMIN_OTP_PAGINACION" data-analytics-action="NAVIGATE" data-analytics-metadata="anterior">Anterior</button><span>Página {filtros.page} de {data?.totalPages ?? 1}</span><button disabled={filtros.page >= (data?.totalPages ?? 1)} onClick={() => setFiltros({ ...filtros, page: filtros.page + 1 })} data-analytics-event="INTERACTION" data-analytics-component="ADMIN_OTP_PAGINACION" data-analytics-action="NAVIGATE" data-analytics-metadata="siguiente">Siguiente</button></nav>
  </div></AppLayout>
}
