  ...
  return (
    <section className="sa-filters" aria-label="Filtros de usuarios">
      <label className="sa-search">
        ...
        <input value={filtros.search} onChange={e => campo('search', e.target.value)} placeholder="Buscar por nombre o correo…" />
      </label>
      <select aria-label="Rol" value={filtros.rol} onChange={e => campo('rol', e.target.value)}>
        <option value="">Todos los roles</option>
        {ROL_OPCIONES.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
      </select>
      <select aria-label="Estado" value={filtros.activo} onChange={e => campo('activo', e.target.value)}>
        <option value="">Todos los estados</option>
        <option value="true">Activos</option>
        <option value="false">Inactivos</option>
      </select>
      <select aria-label="Acceso" value={filtros.bloqueado} onChange={e => campo('bloqueado', e.target.value)}>
